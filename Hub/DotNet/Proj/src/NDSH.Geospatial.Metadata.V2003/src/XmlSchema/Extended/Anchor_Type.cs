...
using NDSH.Geospatial.Metadata.V2003.Properties;
...
namespace NDSH.Geospatial.Metadata.V2003.XmlSchema.Extended {
  ...
  [JsonObject("Anchor")]
  public class Anchor_Type : Model, ISimpleLink, IValue {
      ...
      set {
        ...
        if (_show == null || _show.Equals(value) != true) {
          if (!IShow.ShowBehaviours.Values.ToList().Contains(value)) {
            throw new Exception(Resources.StringNotMemberOfEnumeration); // TODO: Possible Custom Exception??? No
          }
          _show = value;
          OnPropertyChanged();
        }
      ...
      set {
        ...
        if (_actuate == null || _actuate.Equals(value) != true) {
          if (!IActuate.ActuateBehaviours.Values.ToList().Contains(value)) {
            throw new Exception(Resources.StringNotMemberOfEnumeration); // TODO: Possible custom exception??
          }
          _actuate = value;
          OnPropertyChanged();
        }
